...
| analyst | claude-opus-4-6 | yes | Pre-planning consultant that analyzes requests before implementation to identify hidden requirements, edge cases, and potential risks. Use before creating a work plan. |
| architect | claude-opus-4-6 | yes | Read-only consultation agent. High-IQ reasoning specialist for debugging hard problems and high-difficulty architecture design. |
| brand-architect | claude-opus-4-6 | yes | Designs the brand system (Jungian archetype, core metaphor, variation grammar) — self-sufficient discovery; produces .omc/brand/core.md + grammar.md (opus). |
| brand-steward | claude-opus-4-6 | yes | Product constitution owner (opus) — brand identity, tone, visual language governance. |
...
| code-reviewer | claude-opus-4-6 | yes | Expert code review specialist (Opus). Use for comprehensive code quality review. |
| code-simplifier | claude-opus-4-6 | yes | Simplifies and refines code for clarity, consistency, and maintainability (Opus). |
...
| creative-director | claude-opus-4-6 | yes | Brand-variation guardrail — enforces grammar invariants and variance gate on campaign variations; produces per-variation PASS/REVISE/REJECT verdict (opus, read-only). |
| critic | claude-opus-4-6 | yes | Expert reviewer for evaluating work plans against rigorous clarity, verifiability, and completeness standards. Use after planner creates a work plan to validate it before execution. |
...
| domain-expert-reviewer | claude-opus-4-6 | yes | Explicit proxy for domain-expert review — runs multi-persona pre-launch audit and produces a "questions for real expert" list (opus, read-only). |
...
| ideate | claude-opus-4-6 | yes | Divergent idea generator grounded in JTBD/ODI, TRIZ, Blue Ocean, SCAMPER — produces scored, falsifiable hypotheses (opus). |
| performance-guardian | claude-sonnet-4-6 | yes | Performance auditing (sonnet) — Core Web Vitals, bundle size, runtime patterns. |
| planner | claude-opus-4-6 | yes | Strategic planning consultant. Interviews users to understand requirements, then creates comprehensive work plans. NEVER implements - only plans. |
...
| product-ecosystem-architect | claude-opus-4-6 | yes | Product ecosystem architecture (opus) — maps app/content/data/distribution loops and deeper feature paths. |
...
| technology-strategist | claude-opus-4-6 | yes | Technology strategy decision owner (opus) — selects and expands stack choices, application blocks, and skill provisioning targets. |
