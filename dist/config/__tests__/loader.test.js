...
describe("loadConfig() — auto-forceInherit for non-standard providers", () => {
    ...
    it("maps Bedrock family env vars into agent defaults and routing tiers", () => {
        ...
        const config = loadConfig();
        expect(config.agents?.architect?.model).toBe("us.anthropic.claude-opus-4-5-20251101-v1:0");
        ...
        expect(config.routing?.tierModels?.HIGH).toBe("us.anthropic.claude-opus-4-5-20251101-v1:0");
