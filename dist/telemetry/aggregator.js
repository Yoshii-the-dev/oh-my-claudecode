...
export async function aggregate(options) {
    ...
    try {
        ...
        // Compute metrics
        const metrics = computeMetrics(filteredHandoffs, filteredVerdicts, filteredSkills, filteredHooks, filteredLlm, filteredProductCycles, allEvents);
        // Build digest
        const now = new Date();
        ...
        process.stderr.write(`[telemetry/aggregator] error: ${err.message}\n`);
...
function computeMetrics(handoffs, verdicts, skills, hooks, llmInteractions, productCycleEvents, allEvents) {
    // Skip legacy envelopes (no plugin_version)
    let skippedLegacy = 0;
    const hasAttribution = (e) => {
        if (!e['plugin_version']) {
            skippedLegacy++;
            return false;
        }
    ...
    allEvents.forEach(e => { if (!e['plugin_version'])
        skippedLegacy++; });
    // --- agent-handoff
    const handoffCountByType = {};
    ...
    const skillInvocationCount = {};
    const skillLatencySum = {};
    const skillLatencyCount = {};
    ...
    const pluginVersionDistribution = {};
    ...
    for (const e of llmInteractions) {
        ...
        if (cacheRead > 0)
            llmHasCacheData = true;
    }
    // llm_cache_hit_rate = cache_read / tokens_in; null if no cache data or zero tokens
    const llmCacheHitRate = llmHasCacheData && llmTotalTokensIn > 0
...
function computeProductCycleMetrics(events) {
    const eventCounts = {};
    ...
    const automationClosures = events.filter((event) => (event['event'] === 'build_auto_completed'
        || event['event'] === 'research_auto_resumed'));
    const specBuildDecisions = events.filter((event) => (event['event'] === 'stage_decision'
...
function averageBuildCompletionMs(events) {
    const plannedByCycle = new Map();
    const durations = [];
    for (const event of events) {
...
function formatDurationMetric(valueMs) {
    if (valueMs === null)
        return 'n/a';
    if (valueMs < 1000)
...
function renderDigest(windowLabel, generatedAt, metrics, selfImprove) {
    const lines = [];
    // Header
