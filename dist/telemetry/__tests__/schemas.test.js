...
import { validate, validatePayload, agentHandoffSchema, verdictSchema, skillEventsSchema, hookEventsSchema, llmInteractionSchema, productCycleEventsSchema, } from '../schemas.js';
// ---------------------------------------------------------------------------
// validate() — per stream
// ---------------------------------------------------------------------------
...
describe('schemas/validate — edge cases', () => {
    it('returns false with error for unknown stream name', () => {
        const result = validate('unknown-stream', { hook_name: 'x', event: 'y' });
        expect(result.success).toBe(false);
        if (!result.success) {
...
describe('schemas/validatePayload (legacy shim)', () => {
    ...
    it('returns error string for non-object payload', () => {
        const error = validatePayload('hook-events', null);
        expect(typeof error).toBe('string');
    });
