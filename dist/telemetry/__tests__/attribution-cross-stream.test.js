...
describe('M8: multi-stream attribution', () => {
    it('stamps plugin_version, omc_config_hash, and install_id on all 5 streams', async () => {
        ...
        await Promise.all(ALL_STREAMS.map((stream) => emit({
            directory: tmpDir,
            stream,
        ...
        for (const stream of ALL_STREAMS) {
            ...
            expect(typeof envelope['plugin_version'], `stream="${stream}": plugin_version should be a string`).toBe('string');
            expect(envelope['plugin_version'].length, `stream="${stream}": plugin_version should be non-empty`).toBeGreaterThan(0);
            expect(typeof envelope['omc_config_hash'], `stream="${stream}": omc_config_hash should be a string`).toBe('string');
            expect(envelope['omc_config_hash'].length, `stream="${stream}": omc_config_hash should be non-empty`).toBeGreaterThan(0);
            expect(typeof envelope['install_id'], `stream="${stream}": install_id should be a string`).toBe('string');
            expect(envelope['install_id'].length, `stream="${stream}": install_id should be non-empty`).toBeGreaterThan(0);
        }
    });
    it('uses the same install_id across all streams within a single process', async () => {
        ...
        for (const stream of ALL_STREAMS) {
            ...
            installIds.add(envelope['install_id']);
            configHashes.add(envelope['omc_config_hash']);
            pluginVersions.add(envelope['plugin_version']);
        }
...
//# sourceMappingURL=attribution-cross-stream.test.js.map