...
describe('telemetry/writer concurrency', () => {
    let testDir;
    beforeEach(() => {
        testDir = join(tmpdir(), `telemetry-concurrency-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    it('4 concurrent writers × 250 emits = exactly 1000 valid JSONL lines', async () => {
        ...
        await Promise.all(Array.from({ length: WORKERS }, (_, workerIdx) => (async () => {
...
//# sourceMappingURL=writer.concurrency.test.js.map