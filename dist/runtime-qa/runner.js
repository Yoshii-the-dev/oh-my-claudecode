...
export function shouldRunRuntimeQa(root = process.cwd()) {
    if (existsSync(resolve(root, RUNTIME_QA_CONFIG_RELATIVE_PATH)))
        return true;
...
export function detectRuntimeQaConfig(root = process.cwd(), target) {
...
export function writeRuntimeQaRunReport(root, report) {
...
export function renderRuntimeQaRunReport(report) {
    const lines = [
        '# Runtime QA Report',
        '',
    ...
    for (const step of report.step_results) {
        ...
        if (step.stdout_artifact)
            lines.push(`  - stdout_artifact: ${step.stdout_artifact}`);
        if (step.stderr_artifact)
            lines.push(`  - stderr_artifact: ${step.stderr_artifact}`);
    }
    lines.push('');
...
function resolveAdapter(root, config, options) {
...
function defaultAdapterForTarget(target) {
    ...
    if (target === 'cli' || target === 'service')
        return 'cli-tmux';
    if (target === 'mobile')
        return 'mobile-maestro';
    return 'project-script';
}
function inferRuntimeQaTarget(root) {
    ...
    const scriptText = Object.entries(pkg?.scripts ?? {})
        ...
        .join('\n')
        .toLowerCase();
    if (existsSync(resolve(root, '.maestro'))
    ...
    if (/\b(cli|terminal|command-line|tmux)\b/i.test(cycle))
        return 'cli';
    if (/\b(service|server|api smoke)\b/i.test(cycle))
        return 'service';
    return 'project-script';
}
function inferMobileAdapter(root) {
    const scriptText = Object.values(readPackageJson(root)?.scripts ?? {}).join('\n').toLowerCase();
    if (scriptText.includes('detox'))
        return 'mobile-detox';
    if (scriptText.includes('appium'))
...
function detectProjectSmokeCommand(root) {
    const pkg = readPackageJson(root);
    if (!pkg?.scripts)
        return undefined;
...
function expandCommand(name, value) {
    if (!value)
        return [];
...
function hasMobileSmokeCommand(adapter, config, root) {
...
function mobileToolForAdapter(adapter, config) {
    if (config?.mobile?.tool)
        return config.mobile.tool;
    if (adapter === 'mobile-detox')
        return 'detox';
    if (adapter === 'mobile-appium')
        return 'appium';
    return 'maestro';
}
function detectMobileSmokeCommand(adapter, root) {
...
function packageRunCommand(root) {
    if (existsSync(resolve(root, 'pnpm-lock.yaml')))
        return 'pnpm run';
    if (existsSync(resolve(root, 'yarn.lock')))
        return 'yarn';
...
function packageAddCommand(root, manager, packageName) {
    ...
    if (resolved === 'pnpm')
        return `pnpm add ${packageName} --save-dev`;
    if (resolved === 'yarn')
...
function globalPackageAddCommand(manager, packageName) {
    if (manager === 'pnpm')
        return `pnpm add --global ${packageName}`;
    if (manager === 'yarn')
...
function defaultToolDetector(tool, root) {
...
function defaultCommandRunner(command, cwd, timeoutMs) {
    const result = spawnSync(command, {
        cwd,
        shell: true,
        encoding: 'utf-8',
