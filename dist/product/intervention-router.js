...
export function writeProductInterventionHandoff(root, snapshot, plan) {
...
export function readProductInterventionHandoff(root = process.cwd()) {
    const path = resolve(root, PRODUCT_INTERVENTION_HANDOFF_RELATIVE_PATH);
    if (!existsSync(path))
        return undefined;
    return JSON.parse(readFileSync(path, 'utf-8'));
}
function planSpecInterventions(root, snapshot) {
...
function planExperienceGateIntervention(root, snapshot) {
...
function renderProductInterventionHandoff(handoff) {
    const lines = [
...
function interventionContext(plan) {
    return Array.from(new Set(plan.routes.flatMap((route) => route.evidence ?? []))).sort();
}
function planBuildInterventions(root, snapshot) {
...
function needsVisualCreativeProvisioning(root, snapshot) {
    if (!isUserFacingCycle(snapshot))
        return false;
    if (hasVisualCreativeProvisioning(root))
        return false;
    return hasVisualCreativeSignal(root, snapshot);
}
function hasVisualCreativeProvisioning(root) {
...
function hasVisualCreativeSignal(root, snapshot) {
    const corpus = [
        snapshot.cycleGoal,
        snapshot.buildRoute,
...
function evaluateExperienceGateQuality(content) {
    const reasons = [];
    if (!content.trim()) {
...
//# sourceMappingURL=intervention-router.js.map