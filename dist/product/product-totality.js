...
import { PRODUCT_CAPABILITY_GRAPH_JSON_RELATIVE_PATH, PRODUCT_CAPABILITY_GRAPH_MD_RELATIVE_PATH, buildProductCapabilityGraphSnapshot, writeProductCapabilityGraphAudit, } from './capability-graph.js';
import { CYCLE_DOCUMENT_RELATIVE_PATH, CYCLE_PROJECTION_RELATIVE_PATH, parseCycleMarkdown, } from './cycle-document.js';
import { LEARNING_DOCUMENT_RELATIVE_PATH, LEARNING_PROJECTION_RELATIVE_PATH, parseLearningMarkdown, } from './learning-document.js';
import { PORTFOLIO_LEDGER_RELATIVE_PATH, readPortfolioLedger, } from './portfolio-ledger.js';
...
export function generateProductTotalityAudit(root = process.cwd()) {
...
export function renderProductTotalityAudit(report) {
    const scoreRows = Object.entries(report.scores).map(([name, score]) => (`| ${name} | ${score.status} | ${score.value.toFixed(2)} | ${escapeCell(score.detail)} |`));
    const capabilityRows = report.capabilities.map((capability) => (`| ${escapeCell(capability.id)} | ${escapeCell(capability.maturity)} | ${escapeCell(capability.title)} | ${capability.connections.length} | ${capability.missing_depth.length} |`));
    const gapRows = report.gaps.map((gap) => (`| ${gap.severity} | ${gap.code} | ${escapeCell(gap.subject)} | ${escapeCell(gap.recommended_action)} |`));
    const moveRows = report.recommended_moves.map((move) => (`| ${escapeCell(move.id)} | ${move.lane} | ${move.type} | ${escapeCell(move.title)} |`));
    const orphanRows = report.capability_graph.orphan_capabilities.map((orphan) => (`| ${escapeCell(orphan.capability_id)} | ${orphan.severity} | ${orphan.score.toFixed(2)} | ${escapeCell(orphan.reasons.join('; '))} |`));
    return [
...
function collectCycleDocuments(root) {
...
function collectLearningDocuments(root) {
...
function parseCycleJson(root, path) {
...
function parseCycleProjection(root, path) {
...
function safeReadPortfolio(root) {
    try {
...
function readContextArtifacts(root) {
    const candidates = [
...
function capabilityConnections(title, cycle, learning, portfolioMatches, context) {
    const connections = [];
    if (learning)
        connections.push(`learning:${learning.sourceRelativePath}`);
    for (const item of portfolioMatches)
        connections.push(`portfolio:${item.id}`);
    if (context.hasRoadmap && mentions(context.text, title))
        connections.push('roadmap');
    if (context.hasEcosystem)
        connections.push('ecosystem');
    if (context.hasMeaning)
        connections.push('meaning');
...
function missingDepthItems(contract, title, portfolio, context) {
    if (!contract)
        return ['missing feature expectation contract'];
    const missing = [];
    ...
    for (const gate of contract.not_done_until) {
        if (!containsDepthPhrase(context.text, gate))
            missing.push(`not_done_until: ${gate}`);
    }
    return missing;
}
function isDepthRepresented(title, depthText, portfolio, context) {
    if (!depthText.trim())
        return false;
    if (containsDepthPhrase(context.text, depthText))
        return true;
    return (portfolio?.items ?? []).some((item) => containsDepthPhrase(`${item.title} ${item.expected_learning ?? ''} ${item.dependency_unlock ?? ''}`, depthText));
}
function capabilityRisks(cycle, contract, learning, connections, context) {
    const risks = [];
    if (!contract)
        risks.push('completed capability lacks feature expectation; cannot distinguish seed from finished feature');
    if (!learning)
        risks.push('completed capability lacks matching learning capture');
    if (connections.length < 2)
        risks.push('capability appears isolated from roadmap/ecosystem/learning context');
    if ((cycle.spec.build_route === 'product-pipeline' || cycle.spec.build_route === 'both') && !context.hasVisualExpectation) {
...
function buildScores(input) {
    ...
    const connectednessValue = capabilityCount === 0
        ? 0
        : clamp(average(input.capabilities.map((capability) => clamp(capability.connections.length / 4))) * 0.7
    ...
    const depthValue = capabilityCount === 0
        ...
        : average(input.capabilities.map((capability) => {
            if (capability.maturity === 'systemic-v2')
                return 1;
            if (capability.maturity === 'contextual-v1')
                return 0.7;
    ...
    const complexityFitValue = clamp(compositionValue * 0.3
...
function determineStatus(capabilities, gaps, scores) {
    if (capabilities.length === 0)
        return 'empty';
    if (gaps.some((gap) => gap.code === 'orphan-capability'))
        return 'under-connected';
    if (scores.composition.value < 0.35)
        return 'under-composed';
    if (scores.connectedness.value < 0.45)
        return 'under-connected';
    if (gaps.some((gap) => gap.code === 'seeded-capability-needs-depth') || scores.depth.value < 0.55)
        return 'needs-depth';
    return 'balanced';
}
function nextAction(status, gaps) {
    if (status === 'empty')
        return 'Complete one product-cycle, capture learning, then rerun omc product-totality audit --write';
    if (gaps.some((gap) => gap.severity === 'error'))
        return 'Repair totality audit errors, then run omc product-totality audit --write again';
...
function countSupportingSystems(cycles, portfolio) {
    ...
    const portfolioSystems = (portfolio?.items ?? []).filter((item) => (item.status !== 'rejected'
        && (item.lane === 'backend' || item.lane === 'quality' || item.type === 'enabling'))).length;
...
function graphConnectednessScore(graph) {
    if (graph.aggregates.capability_count === 0)
        return 0;
...
function normalizeRelative(root, path) {
    ...
    if (rel === CYCLE_DOCUMENT_RELATIVE_PATH || rel === CYCLE_PROJECTION_RELATIVE_PATH)
        return rel;
    if (rel === LEARNING_DOCUMENT_RELATIVE_PATH || rel === LEARNING_PROJECTION_RELATIVE_PATH)
        return rel;
    return rel;
}
...
//# sourceMappingURL=product-totality.js.map