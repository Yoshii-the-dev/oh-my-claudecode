...
export function getLearningDocumentPath(root = process.cwd()) {
    return resolve(root, LEARNING_DOCUMENT_RELATIVE_PATH);
}
export function getLearningProjectionPath(root = process.cwd()) {
    return resolve(root, LEARNING_PROJECTION_RELATIVE_PATH);
}
export function readLearningDocument(root = process.cwd()) {
    const path = getLearningDocumentPath(root);
    if (!existsSync(path))
        return undefined;
    return JSON.parse(readFileSync(path, 'utf-8'));
}
export function validateLearningDocument(root = process.cwd()) {
    const path = getLearningDocumentPath(root);
    const issues = [];
    if (!existsSync(path)) {
...
export function writeLearningDocument(root, doc) {
...
export function writeLearningProjection(root, document) {
    const doc = document ?? readLearningDocument(root);
    if (!doc)
        throw new Error(`Missing ${LEARNING_DOCUMENT_RELATIVE_PATH}`);
...
export function renderLearningProjection(doc) {
...
export function migrateLearningMarkdownToJson(root = process.cwd(), options = {}) {
...
export function parseLearningMarkdown(content, options = {}) {
...
function parseList(content, key) {
    ...
    if (inline && inline[1] && !inline[1].endsWith(':')) {
        ...
        if (value && value !== '[]' && value !== 'none' && !value.match(/^[-*]/))
            out.push(value);
    }
