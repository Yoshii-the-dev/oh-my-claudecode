...
export function writeProductCapabilityLifecycleAudit(root = process.cwd(), report = generateProductCapabilityLifecycleAudit({ root })) {
...
function lifecycleContextMentions(text, capability, markers) {
    ...
    if (!markers.some((marker) => normalized.includes(normalize(marker))))
        return false;
