...
function isShadowedProjection(path, fileSet) {
    const pairs = [
    ...
    return pairs.some(([projection, source]) => (normalizePath(path).endsWith(projection) && fileSet.has(path.replace(projection, source))));
}
...
function hasFirstUsableLoopSignal(artifact) {
    ...
    if (typed?.kind === 'cycle')
        return typed.value.cycle_stage === 'complete';
    if (typed?.kind === 'learning')
        return Boolean(typed.value.shipped_outcome);
...
function hasUserVisibleWorkSignal(artifact) {
    ...
    if (typed?.kind === 'portfolio') {
        return typed.value.items.some((item) => (item.user_visible === true
            || item.type === 'core-product-slice'
            || ['product', 'ux', 'brand-content', 'distribution'].includes(String(item.lane))));
    }
    if (typed?.kind === 'cycle')
        return typed.value.spec?.build_route === 'product-pipeline' || typed.value.spec?.build_route === 'both';
...
function hasInfrastructureWorkSignal(artifact) {
    const typed = typedArtifact(artifact);
    if (typed?.kind === 'portfolio') {
        return typed.value.items.some((item) => (item.type === 'enabling'
            || ['backend', 'quality'].includes(String(item.lane))));
    }
    if (typed?.kind === 'cycle')
        return typed.value.spec?.build_route === 'backend-pipeline' || typed.value.spec?.build_route === 'both';
...
function typedArtifact(artifact) {
    ...
    if (path.endsWith('.omc/cycles/current.json')) {
        return { kind: 'cycle', value: value };
    }
    if (path.endsWith('.omc/learning/current.json')) {
        return { kind: 'learning', value: value };
    }
    return undefined;
}
