...
export const PRODUCT_ARTIFACT_PATHS = Object.fromEntries(Object.entries(PRODUCT_ARTIFACT_REGISTRY).map(([name, entry]) => [name, entry.currentPath]));
export const PRODUCT_ARTIFACT_CURRENT_PATHS = new Set(Object.values(PRODUCT_ARTIFACT_REGISTRY).map((entry) => entry.currentPath));
...
export function isProductPipelineContractStage(stage) {
    return PRODUCT_PIPELINE_CONTRACT_STAGES.includes(stage);
}
export function getProductArtifactRegistryEntries() {
    return Object.values(PRODUCT_ARTIFACT_REGISTRY);
}
export function renderProductPipelineRegistryMarkdown() {
