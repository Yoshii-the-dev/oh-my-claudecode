...
export const PRODUCT_INTERVENTION_RUN_REPORT_RELATIVE_PATH = '.omc/handoffs/product-cycle-interventions/run-report.json';
const PRODUCT_INTERVENTION_RUN_REPORT_MD_RELATIVE_PATH = '.omc/handoffs/product-cycle-interventions/run-report.md';
export function runProductInterventionExecutionPlan(plan, options = {}) {
...
export function writeProductInterventionRunReport(root, report) {
...
export function renderProductInterventionRunReport(report) {
    const lines = [
    ...
    if (report.step_results.length === 0) {
        ...
        for (const step of report.step_results) {
            ...
            if (step.wait_argv)
                lines.push(`  - wait_argv: ${renderArgv(step.wait_argv)}`);
            if (step.child_job_id)
                lines.push(`  - child_job_id: ${step.child_job_id}`);
            if (step.child_job_status)
                lines.push(`  - child_job_status: ${step.child_job_status}`);
            if (step.exit_code !== undefined)
                lines.push(`  - exit_code: ${step.exit_code}`);
            if (step.wait_exit_code !== undefined)
                lines.push(`  - wait_exit_code: ${step.wait_exit_code}`);
            if (step.signal !== undefined)
                lines.push(`  - signal: ${step.signal ?? 'none'}`);
            if (step.wait_signal !== undefined)
                lines.push(`  - wait_signal: ${step.wait_signal ?? 'none'}`);
        }
    }
    lines.push('');
...
function skipReason(step) {
    if (!step.executable)
        return 'Step is not marked executable.';
    if (!step.argv || step.argv.length === 0)
        return 'Step has no argv to execute.';
...
//# sourceMappingURL=intervention-runner.js.map