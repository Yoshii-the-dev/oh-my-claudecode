...
import { migrateCycleMarkdownToJson, parseCycleMarkdown, readCycleDocument, renderCycleProjection, validateCycleDocument, writeCycleDocument, } from '../cycle-document.js';
let rootsToClean = [];
afterEach(() => {
...
describe('cycle-document', () => {
    ...
    it('round-trips through writeCycleDocument and readCycleDocument', () => {
        const root = createRoot();
        const sample = parseCycleMarkdown(sampleCycleMarkdown(), '2026-04-26T12:00:00Z');
        writeCycleDocument(root, sample);
        const loaded = readCycleDocument(root);
        expect(loaded?.cycle_id).toBe(sample.cycle_id);
        expect(renderCycleProjection(loaded)).toContain('cycle_stage: spec');
    });
});
function createRoot() {
...
//# sourceMappingURL=cycle-document.test.js.map