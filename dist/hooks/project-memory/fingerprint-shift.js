...
export async function scanStaleOmcArtefacts(projectRoot) {
...
export function formatFingerprintShiftWarning(report, previousHashShort, currentHashShort) {
    const lines = [];
    lines.push("<system-reminder>");
    lines.push("[Project Fingerprint Shift Detected]");
    lines.push("Project marker files (package.json, tsconfig, lockfiles, top-level dirs) changed since project memory was last validated.");
    lines.push(`Previous fingerprint: ${previousHashShort} | Current fingerprint: ${currentHashShort}.`);
    if (report.totalCount > 0) {
        lines.push("");
        lines.push("Treat the following `.omc/**` artefacts as POTENTIALLY STALE — they may describe a previous incarnation of this project. Verify against current source files before citing as factual baseline:");
        ...
        lines.push("");
        lines.push("Do not infer implemented features, current phase, selected stack, or roadmap status from these artefacts without matching current filesystem evidence. If in doubt, ask the user whether to keep, archive, or rewrite them.");
    }
    else {
        lines.push("");
...
export async function appendFingerprintHistory(projectRoot, entry) {
    const omcRoot = path.join(projectRoot, ".omc");
...
//# sourceMappingURL=fingerprint-shift.js.map