...
function normalizeCompletionGateInput(record) {
    const tasks = normalizeTaskList(record.tasks) ||
...
export function evaluateCompletionGate(input) {
    ...
    if (pendingTasks > 0)
        reasons.push(`${pendingTasks} pending task(s)`);
    if (inProgressTasks > 0)
        reasons.push(`${inProgressTasks} in-progress task(s)`);
    if (failedTasks > 0)
        reasons.push(`${failedTasks} failed task(s)`);
    if (unknownTasks > 0)
        reasons.push(`${unknownTasks} task(s) with unknown status`);
...
function formatCompletionGateMessage(result) {
    const payload = {
        completion_gate: {
    ...
    return (`[completion-gate] Stop is deferred because the completion gate did not pass.\n\n` +
