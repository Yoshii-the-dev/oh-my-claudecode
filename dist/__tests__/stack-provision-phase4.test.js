...
const { readUsage, writeUsage, recordUsage, recordBatch, applyUsageRecord, findUnusedSkills, summarizeUsage, TELEMETRY_DEFAULTS, } = telemetry;
const { shouldRevalidate, readRevalidationState, writeRevalidationState, revalidateManifest, REVALIDATION_DEFAULTS, } = revalidation;
const { parsePackagesFromCandidate, classifyAdvisorySeverity, gatherCveSignals, CVE_DEFAULTS, } = cveFeed;
...
describe('telemetry: applyUsageRecord', () => {
    ...
    it('increments use_count and updates last_used_at on subsequent records', () => {
        let usage = applyUsageRecord({ schema_version: 1, updated_at: '1970-01-01T00:00:00Z', skills: {} }, { slug: 'foo', now: '2026-04-01T00:00:00.000Z' });
    ...
    it('ignores invalid kind values', () => {
        const usage = applyUsageRecord({ schema_version: 1, updated_at: '1970-01-01T00:00:00Z', skills: {} }, { slug: 'foo', kind: 'evil-kind', now: '2026-04-26T00:00:00.000Z' });
...
describe('revalidation: revalidateManifest', () => {
    it('detects local hash drift and missing files', async () => {
        ...
        const missing = report.results.find((r) => r.slug === 'gone');
...
describe('auto-cleanup: findCleanupCandidates', () => {
    ...
    it('flags idle skills and joins manifest entries when available', async () => {
        ...
        await fs.writeFile(path.join(tmpDir, '.omc', 'provisioned', 'current.json'), JSON.stringify({
            installed: [
                {
                    slug: 'stale',
...
//# sourceMappingURL=stack-provision-phase4.test.js.map