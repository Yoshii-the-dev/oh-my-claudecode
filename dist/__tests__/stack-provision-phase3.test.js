...
const { loadLicensePolicy, resolveLicense, normaliseSpdx, LICENSE_GATE_DEFAULTS, } = licenseGate;
const { collectGlobalSkillRoots, scanGlobalSkills, findDuplicateForCandidate, } = dedup;
const { buildSkillIndex, resolveDependencies, evaluateCapabilityRequirements, } = depGraph;
...
describe('license-gate: resolveLicense', () => {
    it('allows licenses on the allow list', () => {
        ...
        expect(v.flags.some((f) => f.startsWith('license:allow:'))).toBe(true);
    });
    it('denies licenses on the deny list', () => {
        ...
        expect(v.flags.some((f) => f.startsWith('license:denied:'))).toBe(true);
    });
    ...
    it('warns when license is allowed by spdx but not in project allow list', () => {
        ...
        expect(v.decision).toBe('warn');
        expect(v.flags.some((f) => f.startsWith('license:not-allowed:'))).toBe(true);
    });
});
describe('license-gate: loadLicensePolicy', () => {
    ...
    it('reads project policy when present', async () => {
        ...
        await fs.writeFile(path.join(tmpDir, '.omc', 'license.json'), JSON.stringify({
            project_spdx: 'MIT',
...
describe('cross-project-dedup: scanGlobalSkills + findDuplicateForCandidate', () => {
    it('indexes SKILL.md files by frontmatter name and detects sha mismatches', async () => {
        ...
        const driftDup = findDuplicateForCandidate({ slug: 'foo', sha256: 'sha256:deadbeef' }, index);
...
describe('dependency-graph: resolveDependencies', () => {
    ...
    it('blocks peers that are not auto-approve', () => {
        ...
        expect(result.blocked.some((b) => /peer-not-approved:b/.test(b.reason))).toBe(true);
    });
    it('reports peer-missing when slug has no candidate', () => {
        ...
        const result = resolveDependencies(['a'], byId);
        expect(result.blocked.some((b) => /peer-missing:ghost/.test(b.reason))).toBe(true);
    });
});
describe('dependency-graph: evaluateCapabilityRequirements', () => {
    it('returns ok when every requirement is covered', () => {
        const r = evaluateCapabilityRequirements({ requires_capabilities: ['backend/auth', 'data-ai/eval'] }, ['backend/auth', 'data-ai/eval', 'frontend/state']);
        expect(r.ok).toBe(true);
        expect(r.missing).toEqual([]);
    });
    it('flags unsatisfied requirements', () => {
        const r = evaluateCapabilityRequirements({ requires_capabilities: ['backend/auth', 'frontend/state'] }, ['backend/auth']);
...
describe('orchestrate.classifyCandidate (Phase 3 hooks)', () => {
    ...
    it('keeps existing source-blocked behaviour ahead of license check', () => {
        ...
        const verdict = classifyCandidate(c, { ...basePolicy, blocked_sources: ['cskills-sh'] }, { licensePolicy: LICENSE_GATE_DEFAULTS });
        expect(verdict.decision).toBe('reject');
        expect(verdict.reason).toMatch(/blocked by policy/);
    });
});
//# sourceMappingURL=stack-provision-phase3.test.js.map