...
function runJson(script, args, cwd = root) {
...
// ---------------------------------------------------------------------------
function initRun(runRoot, stack, runId) {
...
describe('orchestrate.mjs --network flag parsing', () => {
    ...
    it('still forwards other unknown flags to init.mjs', async () => {
        ...
        expect(opts['__forward__--surfaces']).toBe('mobile');
        expect(opts.network).toBe(true);
    });
});
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=stack-provision-discovery-preflight.test.js.map