...
beforeAll(async () => {
    ...
    const mod = await import(`${PROVISION_SCRIPT}`);
    normalizeToken = mod.normalizeToken;
    createAliasResolver = mod.createAliasResolver;
...
describe('createAliasResolver', () => {
    it('expands built-in aliases for supabase', () => {
        ...
        expect(r.expand('supabase')).toEqual(expect.arrayContaining(['supabase', 'postgres', 'realtime', 'storage']));
    });
    it('expands built-in aliases for trpc', () => {
        const r = createAliasResolver(config, '/tmp/no-project-xyzzy');
        expect(r.expand('trpc')).toEqual(expect.arrayContaining(['trpc', 'rpc', 'typesafe api']));
    });
    ...
    it('merges project override — replace semantics', () => {
        ...
        writeFileSync(join(tmpRoot, '.omc', 'stack-provision', 'aliases.json'), JSON.stringify({ supabase: ['supabase', 'postgres'] }), 'utf8');
    ...
    it('falls back to built-ins on malformed override file', () => {
        ...
        mkdirSync(join(tmpRoot, '.omc', 'stack-provision'), { recursive: true });
        writeFileSync(join(tmpRoot, '.omc', 'stack-provision', 'aliases.json'), '{invalid json', 'utf8');
        const warnings = [];
...
describe('discoveryQueries (new)', () => {
    ...
    it('does not include raw run.contract.stack prose', () => {
        ...
        expect(queries.some((q) => q.includes('ignore-this-prose'))).toBe(false);
    });
    ...
    it('first 5 queries for real stack include expected technologies', () => {
        ...
        const matched = techTerms.filter((t) => queries.some((q) => q.includes(t)));
        expect(matched.length).toBeGreaterThanOrEqual(4);
    });
});
//# sourceMappingURL=stack-provision-query-normalization.test.js.map