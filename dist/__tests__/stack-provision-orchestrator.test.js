...
const ORCHESTRATOR = path.join(REPO_ROOT, 'skills/stack-provision/scripts/orchestrate.mjs');
...
async function runOrchestrator(args, cwd) {
...
describe('orchestrate.mjs — parseArgs', () => {
    ...
    it('forwards unknown long flags via __forward__ keys', () => {
        ...
        expect(a['__forward__--surfaces=backend']).toBe(true);
    });
});
describe('orchestrate.mjs — classifyCandidate', () => {
    ...
    it('forces manual on generated drafts when forbid_generated_drafts is true', () => {
        const v = classifyCandidate({ candidate_id: 'c', source: 'generated', source_trust: 0.99, strict_gate: { passed: true } }, policy);
    ...
    it('rejects candidates whose source is in policy.blocked_sources', () => {
        const v = classifyCandidate({ candidate_id: 'c', source: 'cskills-sh', source_trust: 0.99 }, { ...policy, blocked_sources: ['cskills-sh'] });
...
describe('orchestrate.mjs — applyHeadlessPolicy', () => {
    it('bails when policy is bail and decisions are required', async () => {
        const result = await applyHeadlessPolicy(new Set(['c1']), new Map([['c1', { candidate_id: 'c1', source: 'github' }]]), { headless_action: 'bail' });
        expect(result.cancelled).toBe(true);
        expect(result.bailed).toBe(true);
    });
    it('approves safe candidates under auto_approve_safe', async () => {
        const result = await applyHeadlessPolicy(new Set(['c1']), new Map([
            [
                'c1',
                {
                    candidate_id: 'c1',
...
describe('orchestrate.mjs — e2e plan-only', () => {
    ...
    beforeEach(async () => {
        ...
        const dir = path.join(tmp, '.omc', 'decisions');
        await fs.mkdir(dir, { recursive: true });
        await fs.writeFile(path.join(dir, '2026-04-26-technology-test.md'), '# Tech\n## Chosen Stack\n- next.js\n- react\n');
    });
    afterEach(async () => {
        await fs.rm(tmp, { recursive: true, force: true });
    });
    it('produces a plan-ready completed event and writes the plan file', async () => {
        ...
        const { events, code } = await runOrchestrator(['--plan-only', `--plan-file=${planPath}`, '--headless'], tmp);
        expect(code).toBe(0);
        const completed = events.find((e) => e.event === 'completed');
        expect(completed?.status).toBe('plan-ready');
        expect(completed.plan_path).toBe(planPath);
...
//# sourceMappingURL=stack-provision-orchestrator.test.js.map