...
const { NetworkClient, HostBucket, retryWaitMs, resetDefaultClientForTests, } = network;
...
describe('retryWaitMs', () => {
    it('honors numeric Retry-After (seconds)', () => {
        const res = { headers: { get: (k) => (k === 'retry-after' ? '2' : null) } };
...
describe('NetworkClient — retry on 429', () => {
    ...
    it('throws after retryMax exhaustion', async () => {
        const fetchImpl = async () => mockResponse({ status: 429, headers: { 'retry-after': '0.01' }, body: 'always-rate-limited' });
        const sleeps = [];
        const client = new NetworkClient({
            fetchImpl,
            sleepImpl: async (ms) => { sleeps.push(ms); },
...
//# sourceMappingURL=stack-provision-network-client.test.js.map