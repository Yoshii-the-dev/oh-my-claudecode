...
const { extractGithubRepoPath, externalInstall, evaluateStrictGateForCandidate, strictGatePolicy, renderRegistryInstallCommand, DEFAULT_STRICT_GATE, } = provision;
describe('extractGithubRepoPath', () => {
    it('returns <org>/<repo> from a plain github.com html_url', () => {
        expect(extractGithubRepoPath({ html_url: 'https://github.com/anthropics/courses' })).toBe('anthropics/courses');
    });
    it('extracts <org>/<repo> from a tree URL with subpath', () => {
        expect(extractGithubRepoPath({
    ...
    it('extracts <org>/<repo> from a .git suffix URL', () => {
        expect(extractGithubRepoPath({ url: 'https://github.com/anthropics/courses.git' })).toBe('anthropics/courses');
    });
...
describe('externalInstall — skills-sh source', () => {
    it('promotes skill_md_url candidates to download-skill', () => {
        const inst = externalInstall('skills-sh', { slug: 'foo', skill_md_url: 'https://x/SKILL.md' }, 'foo', `sha256:${'a'.repeat(64)}`);
    ...
    it('derives <org>/<repo> from html_url and produces a valid `npx skills add` command', () => {
        const inst = externalInstall('skills-sh', { slug: 'agent-skills', html_url: 'https://github.com/vercel-labs/agent-skills' }, 'agent-skills');
    ...
    it('honors entry.install_cmd verbatim (registry-supplied command wins)', () => {
        const inst = externalInstall('skills-sh', {
            slug: 'foo',
    ...
    it('falls back to external-command-unresolved when no github URL or skill_md_url is available', () => {
        const inst = externalInstall('skills-sh', { slug: 'supabase-postgres-best-practices' }, 'supabase-postgres-best-practices');
    ...
    it('uses /learn for agentskill-sh source when github URL is available', () => {
        const inst = externalInstall('agentskill-sh', { slug: 'foo', html_url: 'https://github.com/anthropics/foo' }, 'foo');
...
describe('externalInstall — install.method audit field', () => {
    it('annotates every install branch with method and normalized', () => {
        ...
        for (const [inst, expectedMethod, expectedNormalized] of cases) {
            expect(inst.method).toBe(expectedMethod);
            expect(inst.normalized).toBe(expectedNormalized);
        }
    });
});
describe('renderRegistryInstallCommand — github fallback for slug-only entries', () => {
    ...
    it('substitutes the github repo path when entry.html_url is present', () => {
        const cmd = renderRegistryInstallCommand(skillsShRegistry.install_command_template, { slug: 'foo', html_url: 'https://github.com/anthropics/foo' }, 'skills-sh', skillsShRegistry);
        expect(cmd).toBe('npx skills add anthropics/foo');
    });
    it('returns entry.install_cmd verbatim when present, ignoring the template', () => {
        const cmd = renderRegistryInstallCommand(skillsShRegistry.install_command_template, { slug: 'foo', install_cmd: 'npx skills add a/b --pin v2' }, 'skills-sh', skillsShRegistry);
        expect(cmd).toBe('npx skills add a/b --pin v2');
    });
    it('falls back to slug only when no github reference is available', () => {
        const cmd = renderRegistryInstallCommand(skillsShRegistry.install_command_template, { slug: 'orphan-skill' }, 'skills-sh', skillsShRegistry);
...
//# sourceMappingURL=stack-provision-install-target.test.js.map