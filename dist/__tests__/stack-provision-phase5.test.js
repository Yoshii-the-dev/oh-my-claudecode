...
const { readPinIndex, pinSkill, checkPinDrift, formatUnifiedDiff, TOFU_DEFAULTS, } = tofu;
const { parseFrontmatter, detectToolCallouts, detectShellCommands, detectUrls, detectEnvVarReads, buildBehaviorFingerprint, classifyDryRunRisk, dryRunSkill, SANDBOX_DRYRUN_DEFAULTS, } = sandbox;
const { annotateTofuDrift, annotateSandboxFingerprints, pinApprovedSkills, DEFAULT_POLICY, } = orchestrate;
let tmpDir;
beforeEach(async () => {
...
describe('tofu-pinning: pinSkill + readPinIndex round-trip', () => {
    it('records pin entry with sha + snapshot', async () => {
        ...
        const snap = await fs.readFile(path.join(tmpDir, entry.snapshot_path), 'utf8');
...
describe('sandbox-dryrun: detectors', () => {
    it('detects known tool callouts', () => {
        ...
        expect(tools.map((t) => t.tool).sort()).toEqual(['Bash', 'Read']);
    });
    it('extracts shell commands from fenced bash blocks', () => {
        ...
        expect(cmds.map((c) => c.command).sort()).toEqual(['git', 'npm']);
    });
...
describe('sandbox-dryrun: buildBehaviorFingerprint + classifyDryRunRisk', () => {
    ...
    it('escalates dangerous commands to critical', () => {
        ...
        expect(verdict.flags.some((f) => /curl-pipe-shell:critical/.test(f))).toBe(true);
    });
    ...
    it('flags sensitive env reads as warn', () => {
        ...
        const verdict = classifyDryRunRisk(buildBehaviorFingerprint(text));
        expect(verdict.severity).toBe('warn');
        expect(verdict.flags.some((f) => /sensitive-env:GITHUB_TOKEN/.test(f))).toBe(true);
    });
...
describe('orchestrate.annotateTofuDrift', () => {
    ...
    it('uses critical flag in strict mode', async () => {
        ...
        const candidates = [{ candidate_id: 'b', slug, preview: 'B' }];
    ...
    it('is a no-op when tofu_enabled=false', async () => {
        const slug = 'noop';
        await pinSkill({ projectRoot: tmpDir, slug, content: 'A' });
        const candidates = [{ candidate_id: 'n', slug, preview: 'B' }];
        const result = await annotateTofuDrift({
            candidates,
            projectRoot: tmpDir,
...
describe('orchestrate.annotateSandboxFingerprints', () => {
    it('annotates each candidate with severity + fingerprint', () => {
        ...
        expect(candidates[1].risk_flags?.some((f) => /sandbox:rm-rf-root:critical/.test(f))).toBe(true);
    });
...
//# sourceMappingURL=stack-provision-phase5.test.js.map