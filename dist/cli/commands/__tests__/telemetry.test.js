...
function makeEvent(stream, overrides = {}) {
...
describe('CLI: omc telemetry digest', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(eventsDir, 'hook-events.jsonl'), [
            makeEvent('hook-events', { hook_name: 'factcheck', event: 'fired' }),
            makeEvent('hook-events', { hook_name: 'keyword-detector', event: 'matched' }),
        ].join('\n') + '\n', 'utf-8');
        writeFileSync(join(eventsDir, 'agent-handoff.jsonl'), makeEvent('agent-handoff', { event: 'start', agent_type: 'executor' }) + '\n', 'utf-8');
        writeFileSync(join(eventsDir, 'verdict.jsonl'), makeEvent('verdict', { event: 'verdict', agent_type: 'executor', verdict: 'propose' }) + '\n', 'utf-8');
