...
import { buildAutoAttemptKey, decideAutoAction, } from '../../product/auto-decision.js';
import { incrementProductCycleAutoAttempt, readProductCycleAutoState, startProductCycleAutoState, updateProductCycleAutoState, } from '../../product/cycle-auto-state.js';
...
export async function productCycleRunCommand(root, options, logger = console) {
    ...
    const autoResult = autoPolicy === 'safe'
        ...
        : { finalReport: report, autoRuns: [], autoBuild: undefined };
    ...
    const autoFailed = autoResult.autoRuns.some((run) => (run.decision.action === 'block'
...
async function emitProductCycleAutoRunTelemetry(root, autoRuns, autoPolicy) {
    const context = productCycleTelemetryContext(root);
...
function runSafeAutoCycle(root, initialReport, options, autoPolicy) {
    const rootPath = root ?? process.cwd();
    ...
    for (let loop = 0; loop < maxAttempts * 3; loop += 1) {
        ...
        if (report.researchHandoff) {
            ...
            if (!resumedCycleReport || runReport.status === 'failed')
                break;
        ...
        if (report.interventionExecutionPlan) {
            ...
            if (!resumedCycleReport || runReport.status !== 'passed')
                break;
            report = resumedCycleReport;
            continue;
        }
...
function isAutoPolicyValue(value) {
    const normalized = value.trim().toLowerCase();
    return normalized === 'off' || normalized === 'safe';
}
function renderRunReport(report, autoBuild, autoRuns = []) {
    const lines = [];
