...
export async function stateHygieneCommand(root, options, logger = console) {
...
export function runStateHygiene(root, options = {}) {
...
export function inspectStateHygiene(root) {
...
export function isOmcRuntimeStatePath(path) {
    ...
    if (segments.indexOf('.omc', 1) !== -1)
        return true;
    const secondSegment = segments[1];
    if (!secondSegment)
        return false;
    if (ROOT_RUNTIME_DIRECTORIES.has(secondSegment))
        return true;
    if (segments.length === 2 && ROOT_RUNTIME_FILES.has(secondSegment))
        return true;
    return false;
}
...
function untrackFiles(gitRoot, paths) {
    const chunkSize = 100;
...
function renderStateHygieneReport(report) {
    const lines = [
...
//# sourceMappingURL=state-hygiene.js.map