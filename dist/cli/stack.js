...
function prettyEvent(evt) {
    switch (evt.event) {
        case 'started':
            return chalk.gray(`stack-provision starting (project: ${evt.project_root})`);
        case 'phase': {
            ...
            if (status === 'failed')
                return chalk.red(`  ✗ ${name}: ${evt.message ?? 'failed'}`);
            return null;
        }
        case 'detect': {
            const source = String(evt.source);
            if (source === 'adr')
                return chalk.gray(`  detected ADR: ${evt.path}`);
            if (source === 'stack-list')
                return chalk.gray(`  using stack list: ${evt.stack}`);
        ...
        case 'review_summary': {
            ...
            return chalk.cyan(`  review: ${total} candidates → ${auto} auto-approve, ${need} need decision, ${rej} rejected`);
        }
        case 'install_plan': {
            ...
            return chalk.cyan(`  install plan: ${approved} approved, ${skipped} skipped, ${editReq} need edit`);
        }
        case 'completed': {
            const status = String(evt.status);
            if (status === 'success')
                return chalk.green(`✓ done — ${evt.manifest_path ?? evt.run_dir}`);
            if (status === 'plan-ready')
                return chalk.green(`✓ plan ready — ${evt.plan_path}`);
            if (status === 'no-op')
                return chalk.yellow(`= no-op: ${evt.reason}`);
            if (status === 'cancelled')
                return chalk.yellow(`= cancelled: ${evt.reason}`);
            if (status === 'paused-for-edits') {
                const edits = Array.isArray(evt.edit_paths) ? evt.edit_paths : [];
                return chalk.yellow(`= paused — edit then re-run:\n${edits.map((p) => `    ${p}`).join('\n')}`);
            }
            if (status === 'partial')
                return chalk.yellow(`= partial: verify failed; manifest at ${evt.manifest_path}`);
            if (status === 'review-only')
                return chalk.green(`✓ review only — ${evt.run_dir}`);
            return chalk.gray(`completed: ${status}`);
        }
        case 'error':
            return chalk.red(`✗ error: ${evt.message}`);
        default:
            return null;
    }
}
function buildOrchestratorArgs(parsed) {
    const args = [];
    if (parsed.subcommand === 'plan')
        args.push('--plan-only');
    if (parsed.subcommand === 'apply')
        args.push('--apply');
    for (const a of parsed.forwardArgs)
        args.push(a);
    for (const p of parsed.positional)
        args.push(p);
    if (!args.includes('--headless') && !args.includes('--interactive'))
        args.push('--headless');
    return { args };
}
export async function stackCommand(argv) {
    ...
    if (!existsSync(orchestrator)) {
        process.stderr.write(chalk.red(`[omc stack] orchestrator not found at ${orchestrator}.\n`) +
