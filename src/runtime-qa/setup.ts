...
import {
  diagnoseRuntimeQa,
  type RuntimeQaDoctorOptions,
  type RuntimeQaDoctorReport,
} from './diagnostics.js';
import {
...
export function setupRuntimeQaPrerequisites(root = process.cwd(), options: RuntimeQaSetupOptions = {}): RuntimeQaSetupReport {
...
export function buildRuntimeQaSetupPlan(
  root: string,
  doctor: RuntimeQaDoctorReport,
  options: Pick<RuntimeQaSetupOptions, 'platform'> = {},
): RuntimeQaSetupStep[] {
  const platform = options.platform ?? process.platform;
  const steps: RuntimeQaSetupStep[] = [];
  const rawConfig = readRuntimeQaConfigRaw(root) as {
    target?: unknown;
    adapter?: unknown;
  } | undefined;
...
function java17SetupStep(platform: NodeJS.Platform): RuntimeQaSetupStep {
  if (platform === 'darwin') {
    return {
      id: 'java-17-install',
...
function maestroSetupSteps(platform: NodeJS.Platform): RuntimeQaSetupStep[] {
  if (platform === 'darwin') {
    return [
      {
        id: 'maestro-tap',
...
function playwrightBrowsersStep(root: string): RuntimeQaSetupStep {
...
function androidSdkSetupStep(platform: NodeJS.Platform): RuntimeQaSetupStep {
  if (platform === 'darwin') {
    return {
...
function hasIssue(report: RuntimeQaDoctorReport, code: string): boolean {
  return report.issues.some((issue) => issue.code === code);
}

function dedupeSteps(steps: RuntimeQaSetupStep[]): RuntimeQaSetupStep[] {
...
function defaultSetupCommandRunner(command: string, cwd: string, timeoutMs: number): CommandRunResult {
