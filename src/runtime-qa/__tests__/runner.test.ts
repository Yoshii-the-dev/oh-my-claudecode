...
describe('runtime QA runner', () => {
  it('runs configured project-script commands and writes compact evidence', () => {
    ...
    const calls: string[] = [];
    const commandRunner: RuntimeQaCommandRunner = (command) => {
      calls.push(command);
  ...
  it('detects mobile smoke declarations and missing mobile tooling', () => {
    ...
    const commandRunner: RuntimeQaCommandRunner = () => ({ status: 0, stdout: 'passed', stderr: '' });
    const toolDetector: RuntimeQaToolDetector = (tool) => ({
      tool,
      detected: true,
      method: 'test',
  ...
  it('migrates legacy mobile flow configs while preserving policy metadata', () => {
    ...
    const migrated = JSON.parse(readFileSync(join(root, '.omc/runtime-qa.json'), 'utf-8')) as {
      target: string;
      adapter: string;
      commands: { build: string[]; smoke: string[] };
