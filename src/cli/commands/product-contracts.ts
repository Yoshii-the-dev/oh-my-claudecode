...
import {
  validateProductPipelineContracts,
  type ProductPipelineValidationReport,
} from '../../product/pipeline-contract-validator.js';
import {
  PRODUCT_PIPELINE_CONTRACT_STAGES,
  isProductPipelineContractStage,
} from '../../product/pipeline-registry.js';
...
export async function productContractsCommand(
  root: string | undefined,
  options: ProductContractsCommandOptions,
): Promise<number> {
...
function renderReport(report: ProductPipelineValidationReport): void {
...
function summarizeMetrics(metrics: Record<string, number | string | boolean>): string {
