...
describe('product cycle CLI command', () => {
  ...
  it('previews safe product-cycle repairs without writing projections', async () => {
    ...
    try {
      ...
      const report = JSON.parse(String(logger.log.mock.calls[0]?.[0])) as {
        ok: boolean;
  ...
  it('auto-runs eligible build pipeline team jobs and resumes the cycle', async () => {
    ...
    try {
      ...
      const commandRunner = vi.fn((argv: string[]) => {
      ...
      const output = JSON.parse(String(logger.log.mock.calls[0]?.[0] ?? '{}')) as {
        autoPolicy?: string;
        autoBuild?: {
          status: string;
          runReport?: { status: string };
          resumedCycleReport?: { stoppedReason: string; endedAtStage?: string };
        };
      };
  ...
  it('resumes product-cycle run after research-run finds a valid artifact', async () => {
    ...
    try {
      ...
      expect(exitCode).toBe(0);
      const output = JSON.parse(String(logger.log.mock.calls[0]?.[0] ?? '{}')) as {
        report: { research_artifact_valid: boolean };
        resumedCycleReport: { stoppedReason: string; pauseInstruction?: string } | null;
      };
...
function cycleArtifactForRepair(stage: string): string {
...
function learningArtifactForRepair(): string {
...
function portfolioLedgerForRepair(): string {
...
function featureExpectationForRepair(): Record<string, unknown> {
  return {
    user_job: 'resume a real pattern-reading session without losing place',
    first_meaningful_use: 'open a sample pattern, advance rows, close, and resume on the next row',
...
function writeBuildCycle(root: string): void {
...
function writeBackendResearch(root: string): void {
...
function cycleAtBuildArtifact(): string {
