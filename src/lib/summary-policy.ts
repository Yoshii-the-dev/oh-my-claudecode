...
export function normalizeSummaryPolicy(policy?: SummaryPolicyConfig): SummaryPolicy {
  return {
...
export function truncateInlineLog(value: string | undefined, policy?: SummaryPolicyConfig): string {
  const text = value ?? '';
...
export function limitReportLines(
  value: string,
  maxLines: number,
  omittedLabel = 'lines omitted by summaryPolicy',
): string {
...
export function limitFinalReport(value: string, policy?: SummaryPolicyConfig): string {
  return limitReportLines(value, normalizeSummaryPolicy(policy).finalReportMaxLines);
}

export function limitStageReport(value: string, policy?: SummaryPolicyConfig): string {
...
function isSummaryMode(value: unknown): value is SummaryPolicy['mode'] {
