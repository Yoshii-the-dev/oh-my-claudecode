...
export type ProductCapabilityLifecycleHistoryEventType =
  | 'observed'
  | 'stage-transition'
  | 'decision-change';

export type ProductCapabilityLifecycleTrend =
  | 'new'
  | 'progressed'
  | 'regressed'
  | 'triaged'
...
export function writeProductCapabilityLifecycleAudit(
  ...
  history = generateProductCapabilityLifecycleHistory({ root, current: report }),
): { jsonPath: string; mdPath: string; historyJsonPath: string; historyMdPath: string } {
...
export function readProductCapabilityLifecycleHistory(
  root = process.cwd(),
): ProductCapabilityLifecycleHistoryReport | undefined {
  const path = resolve(root, PRODUCT_CAPABILITY_LIFECYCLE_HISTORY_JSON_RELATIVE_PATH);
  if (!existsSync(path)) return undefined;
  try {
    const parsed = JSON.parse(readFileSync(path, 'utf-8')) as ProductCapabilityLifecycleHistoryReport;
...
export function generateProductCapabilityLifecycleHistory(
  options: ProductCapabilityLifecycleHistoryOptions,
): ProductCapabilityLifecycleHistoryReport {
  ...
  const previousStateByCapability = new Map<string, ProductCapabilityLifecycleHistoryCapability>();
  for (const capability of previous?.capabilities ?? []) {
    previousStateByCapability.set(capability.capability_id, capability);
  }
  const newEvents: ProductCapabilityLifecycleHistoryEvent[] = [];
...
export function writeProductCapabilityLifecycleHistory(
  root = process.cwd(),
...
export function renderProductCapabilityLifecycleHistory(report: ProductCapabilityLifecycleHistoryReport): string {
  const capabilityRows = report.capabilities.map((capability) => (
    `| ${escapeCell(capability.capability_id)} | ${capability.previous_stage ?? 'new'} | ${capability.current_stage} | ${capability.trend} | ${escapeCell(capability.transition_path.join(' -> '))} |`
  ));
  const eventRows = report.events.slice(-20).map((event) => (
    `| ${event.recorded_at} | ${event.capability_id} | ${event.from_stage ?? 'new'} -> ${event.to_stage} | ${event.trend} | ${escapeCell(event.reason)} |`
  ));
...
function previousCapabilityStateFromEvents(
  events: ProductCapabilityLifecycleHistoryEvent[],
  capabilityId: string,
): ProductCapabilityLifecycleHistoryCapability | undefined {
...
function lifecycleHistoryEvent(
  capability: ProductCapabilityLifecycleItem,
  previous: ProductCapabilityLifecycleHistoryCapability | undefined,
  context: { recordedAt: string; reportGeneratedAt: string; sequence: number },
): ProductCapabilityLifecycleHistoryEvent | undefined {
  ...
  if (previous && !stageChanged && !decisionChanged) return undefined;

  const eventType: ProductCapabilityLifecycleHistoryEventType = !previous
    ? 'observed'
    : stageChanged
      ? 'stage-transition'
      : 'decision-change';
...
function classifyLifecycleTransition(
  from: ProductCapabilityLifecycleStage | undefined,
  to: ProductCapabilityLifecycleStage,
): ProductCapabilityLifecycleTrend {
  if (!from) return 'new';
...
function lifecycleStageRank(stage: ProductCapabilityLifecycleStage): number {
...
function compactStagePath(
  stages: ProductCapabilityLifecycleStage[],
  currentStage: ProductCapabilityLifecycleStage,
): ProductCapabilityLifecycleStage[] {
  const compact: ProductCapabilityLifecycleStage[] = [];
...
function formatHistoryTransition(event: ProductCapabilityLifecycleHistoryEvent): string {
  return `${event.from_stage ?? 'new'} -> ${event.to_stage}`;
}

function historyNextAction(capabilities: ProductCapabilityLifecycleHistoryCapability[]): string {
  ...
  const v0Pressure = capabilities.filter((capability) => (
    capability.current_stage === 'seeded' || capability.current_stage === 'proving'
  )).length;
  const durableMass = capabilities.filter((capability) => (
    capability.current_stage === 'connected' || capability.current_stage === 'mature'
  )).length;
