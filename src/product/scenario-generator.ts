...
export type ProductScenarioGenerationStatus =
  | 'empty'
  | 'needs-expectation'
  | 'partial'
  | 'ready';

export type ProductGeneratedScenarioPhase =
  | 'setup'
  | 'start'
  | 'core-action'
  | 'state-change'
...
export function generateProductScenarioPlan(root = process.cwd(), now = new Date()): ProductScenarioGenerationReport {
...
export function writeProductScenarioPlan(
  root = process.cwd(),
  report = generateProductScenarioPlan(root),
...
export function readProductScenarioPlan(root = process.cwd()): ProductScenarioGenerationReport | undefined {
  const path = resolve(root, PRODUCT_SCENARIO_GENERATOR_JSON_RELATIVE_PATH);
  if (!existsSync(path)) return undefined;
  try {
    return JSON.parse(readFileSync(path, 'utf-8')) as ProductScenarioGenerationReport;
  } catch {
    return undefined;
  }
}
...
export function renderProductScenarioPlan(report: ProductScenarioGenerationReport): string {
  const rows = report.scenarios.map((scenario) => (
    `| ${escapeCell(scenario.id)} | ${escapeCell(scenario.capability_title)} | ${escapeCell(scenario.first_meaningful_use)} | ${scenario.steps.length} |`
  ));
...
function collectCycleScenarioSources(root: string): CycleScenarioSource[] {
  const byCycle = new Map<string, CycleScenarioSource>();
  for (const source of [
    readCurrentCycleSource(root),
    ...readDatedCycleJsonSources(root),
  ].filter((source): source is CycleScenarioSource => Boolean(source))) {
...
function readCurrentCycleSource(root: string): CycleScenarioSource | undefined {
...
function readDatedCycleJsonSources(root: string): CycleScenarioSource[] {
  const cyclesDir = resolve(root, '.omc/cycles');
  if (!existsSync(cyclesDir)) return [];
  const sources: CycleScenarioSource[] = [];
  for (const entry of readdirSync(cyclesDir)) {
    ...
    try {
      const document = JSON.parse(readFileSync(path, 'utf-8')) as CycleDocument;
...
function buildScenario(source: CycleScenarioSource): ProductGeneratedScenario {
  const doc = source.document;
  const contract = doc.spec.feature_expectation_contract as CycleFeatureExpectationContract;
  ...
  const steps: ProductGeneratedScenarioStep[] = [
...
function buildGaps(
  sources: CycleScenarioSource[],
  scenarios: ProductGeneratedScenario[],
): ProductScenarioGenerationGap[] {
  const gaps: ProductScenarioGenerationGap[] = [];
  if (sources.length === 0) {
    gaps.push({
      severity: 'warning',
...
function hasUsableExpectation(contract: CycleFeatureExpectationContract | undefined): contract is CycleFeatureExpectationContract {
...
function determineStatus(
  ...
  gaps: ProductScenarioGenerationGap[],
): ProductScenarioGenerationStatus {
...
function nextAction(status: ProductScenarioGenerationStatus): string {
