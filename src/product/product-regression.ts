...
import {
  generateHistoricalScorecard,
  type HistoricalScorecardReport,
} from './historical-scorecard.js';
import {
...
export type ProductRegressionStatus =
  | 'empty'
  | 'needs-repair'
  | 'needs-scenario-proof'
  | 'carrying-debt'
  | 'stable';

export type ProductRegressionDebtCategory =
  | 'learning'
  | 'capability-depth'
  | 'orphan'
  | 'scenario-proof'
...
export function generateProductRegressionAudit(options: ProductRegressionOptions = {}): ProductRegressionReport {
...
export function writeProductRegressionAudit(
  root = process.cwd(),
  report = generateProductRegressionAudit({ root }),
...
export function renderProductRegressionAudit(report: ProductRegressionReport): string {
  const debtRows = report.debts.map((debt) => (
    `| ${debt.severity} | ${debt.category} | ${escapeCell(debt.subject)} | ${escapeCell(debt.recommended_action)} |`
  ));
...
function learningDebts(
  historical: HistoricalScorecardReport,
  totality: ProductTotalityReport,
  scenarioCoverage: ProductScenarioCoverageReport,
): ProductRegressionDebt[] {
  const debts: ProductRegressionDebt[] = [];
...
function capabilityDebts(totality: ProductTotalityReport): ProductRegressionDebt[] {
  const debts: ProductRegressionDebt[] = [];
...
function scenarioDebts(scenarioCoverage: ProductScenarioCoverageReport): ProductRegressionDebt[] {
...
function historicalTrendDebts(historical: HistoricalScorecardReport): ProductRegressionDebt[] {
...
function capabilityDebt(
  capability: ProductTotalityCapability,
  severity: ProductRegressionDebt['severity'],
  category: ProductRegressionDebtCategory,
  message: string,
...
function determineStatus(cycleCount: number, debts: ProductRegressionDebt[]): ProductRegressionStatus {
...
function nextAction(status: ProductRegressionStatus): string {
...
function dedupeDebts(debts: ProductRegressionDebt[]): ProductRegressionDebt[] {
  const seen = new Set<string>();
  const out: ProductRegressionDebt[] = [];
