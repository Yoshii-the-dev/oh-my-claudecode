...
export type ProductScenarioCoverageStatus =
  | 'empty'
  | 'missing-scenarios'
...
export type ProductScenarioCoverageLevel =
  | 'missing'
  | 'declared'
  | 'runtime-passed'
  | 'runtime-failed'
...
export function generateProductScenarioCoverageAudit(
  options: ProductScenarioCoverageOptions = {},
...
export function writeProductScenarioCoverageAudit(
  root = process.cwd(),
  report = generateProductScenarioCoverageAudit({ root }),
...
export function renderProductScenarioCoverageAudit(report: ProductScenarioCoverageReport): string {
  const scenarioRows = report.scenarios.map((scenario) => (
    `| ${escapeCell(scenario.id)} | ${scenario.coverage} | ${escapeCell(scenario.capability_title)} | ${escapeCell(scenario.expected_user_loop)} | ${scenario.gaps.length} |`
  ));
...
function buildScenarioGaps(
  scenarios: ProductScenarioCoverageScenario[],
  totality: ProductTotalityReport,
): ProductScenarioCoverageGap[] {
  const gaps: ProductScenarioCoverageGap[] = [];
...
function nextAction(status: ProductScenarioCoverageStatus, gaps: ProductScenarioCoverageGap[]): string {
...
function recommendedAction(capability: ProductTotalityCapability, coverage: ProductScenarioCoverageLevel): string {
...
function runtimeConfigMatches(config: RuntimeQaConfig | undefined, scenarioText: string): boolean {
...
function runtimeHandoffMatches(
  report: RuntimeQaRunReport | undefined,
  capability: ProductTotalityCapability,
  scenarioText: string,
): boolean {
  if (!report) return false;
  if (report.cycle_id && report.cycle_id === capability.source_cycle) return true;
  const steps = report.step_results.map((step) => `${step.name} ${step.command ?? ''} ${step.reason}`).join('\n');
  return fuzzyMentions(steps, scenarioText);
}

function fuzzyMentions(haystack: string, needle: string): boolean {
...
function readRuntimeQaHandoff(root: string): RuntimeQaRunReport | undefined {
  const path = resolve(root, RUNTIME_QA_HANDOFF_RELATIVE_PATH);
  if (!existsSync(path)) return undefined;
  try {
    const parsed = JSON.parse(readFileSync(path, 'utf-8')) as RuntimeQaRunReport;
...
function dedupeGaps(gaps: ProductScenarioCoverageGap[]): ProductScenarioCoverageGap[] {
  const seen = new Set<string>();
  const out: ProductScenarioCoverageGap[] = [];
