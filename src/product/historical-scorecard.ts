...
import { parseCycleMarkdown, type CycleDocument } from './cycle-document.js';
import { parseLearningMarkdown, type LearningDocument } from './learning-document.js';
...
export interface HistoricalTrend {
  metric: string;
  values: Array<{ cycleId: string; value: number | null }>;
...
export function generateHistoricalScorecard(root = process.cwd()): HistoricalScorecardReport {
...
function collectCycleDocuments(artifactRoot: string): RawCycleDocument[] {
  ...
  const seen = new Map<string, RawCycleDocument>();
  ...
  for (const entry of entries) {
    ...
    if (entry.endsWith('.json')) {
      const document = parseJsonDocument<CycleDocument>(filePath);
...
function buildCycleSummary(artifactRoot: string, entry: RawCycleDocument): HistoricalCycleSummary {
...
function readMatchingLearning(artifactRoot: string, cycleId: string): LearningDocument | undefined {
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.endsWith('.json')) {
      const json = parseJsonDocument<LearningDocument>(path);
      if (json) return json;
...
function countCheckedStages(document: CycleDocument): number {
  if (!document.stage_checklist) return 0;
  return Object.values(document.stage_checklist).filter(Boolean).length;
}

function inferStartDate(document: CycleDocument, entry: RawCycleDocument): string | undefined {
...
function inferCompletedDate(document: CycleDocument, entry: RawCycleDocument): string | undefined {
...
function timeToCompleteDays(start: string | undefined, end: string | undefined): number | null {
...
function scoreWorkLanes(document: CycleDocument): { userVisible: number; infrastructure: number } {
  const slices: string[] = [
...
function buildTrends(cycles: HistoricalCycleSummary[]): HistoricalTrend[] {
  return [
...
function buildTrend(
  metric: string,
  cycles: HistoricalCycleSummary[],
  selector: (cycle: HistoricalCycleSummary) => number | null,
  direction: 'higher-better' | 'lower-better',
): HistoricalTrend {
  const values = cycles.map((cycle) => ({ cycleId: cycle.cycle_id, value: selector(cycle) }));
  const numeric = values.filter((entry): entry is { cycleId: string; value: number } => typeof entry.value === 'number' && Number.isFinite(entry.value));
  if (numeric.length < 2) {
...
function buildComparison(cycles: HistoricalCycleSummary[]): HistoricalComparison {
  if (cycles.length === 0) {
...
function parseJsonDocument<T>(path: string): T | undefined {
