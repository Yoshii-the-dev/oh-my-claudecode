...
import { inspectUnifiedMcpRegistrySync, type UnifiedMcpRegistryStatus } from '../installer/mcp-registry.js';
import type { PluginConfig } from '../shared/types.js';
import { atomicWriteJsonSync } from '../lib/atomic-write.js';

export type FeatureGenerationStatus = 'ready' | 'needs-input' | 'needs-mcp' | 'blocked';

export type FeatureGenerationSourceKind =
  | 'vision'
  | 'research'
  | 'competitors'
  | 'meaning'
...
const SOURCE_CANDIDATES: Record<FeatureGenerationSourceKind, string[]> = {
...
export function planFeatureGeneration(options: FeatureGenerationOptions = {}): FeatureGenerationPlan {
  const root = resolve(options.root ?? process.cwd());
  const now = options.now ?? new Date();
  const staleAfterDays = options.staleAfterDays ?? DEFAULT_STALE_AFTER_DAYS;
  const mcpStatus = options.mcpStatus ?? inspectUnifiedMcpRegistrySync();
  const config = options.config ?? loadConfig();
  const sources = (Object.keys(SOURCE_CANDIDATES) as FeatureGenerationSourceKind[])
...
export function writeFeatureGenerationPlan(root = process.cwd(), plan: FeatureGenerationPlan): {
  jsonPath: string;
  mdPath: string;
} {
...
export function renderFeatureGenerationPlan(plan: FeatureGenerationPlan): string {
  const lines = [
...
function scoreSources(sources: FeatureGenerationSource[]): number {
  return sources.reduce((sum, source) => {
    if (source.status === 'present') return sum + 1;
    if (source.status === 'thin' || source.status === 'stale') return sum + 0.5;
    return sum;
  }, 0);
}

function buildMcpRecommendations(
  sources: FeatureGenerationSource[],
  status: UnifiedMcpRegistryStatus,
  config: PluginConfig,
): FeatureGenerationMcpRecommendation[] {
  const serverNames = new Set(status.serverNames);
  const source = (kind: FeatureGenerationSourceKind) => sources.find((entry) => entry.kind === kind);
...
function buildBlockers(sources: FeatureGenerationSource[], sourceScore: number, goal: string | undefined): string[] {
...
function buildRecommendedCommands(
  ...
  sources: FeatureGenerationSource[],
  missingRequiredMcp: FeatureGenerationMcpRecommendation[],
): string[] {
