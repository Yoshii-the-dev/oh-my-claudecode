...
export type ProductCapabilityGraphEdgeType =
  | 'learning'
  | 'portfolio'
  | 'context'
  | 'shared-job'
...
export function buildProductCapabilityGraphSnapshot(
  capabilities: ProductTotalityCapability[],
): ProductCapabilityGraphSnapshot {
  const nodes = new Map<string, ProductCapabilityGraphNode>();
  const edges: ProductCapabilityGraphEdge[] = [];
  ...
  const orphanCapabilities = capabilities
    .map((capability) => orphanForCapability(
      capability,
      capabilityPeerDegrees.get(capability.id) ?? 0,
      capabilities.length,
    ))
    .filter((orphan): orphan is ProductCapabilityGraphOrphan => orphan !== undefined);
  ...
  const capabilityEdgeCount = edges.filter((edge) => (
    capabilityDegrees.has(edge.from) && capabilityDegrees.has(edge.to)
  )).length;
...
export function writeProductCapabilityGraphAudit(
  root: string,
  snapshot: ProductCapabilityGraphSnapshot,
  metadata: {
    generatedAt: string;
  ...
  const report: ProductCapabilityGraphReport = {
    schema_version: 1,
...
export function renderProductCapabilityGraph(report: ProductCapabilityGraphReport | ProductCapabilityGraphSnapshot): string {
  ...
  const nodeRows = report.nodes.map((node) => (
    `| ${escapeCell(node.id)} | ${node.kind} | ${escapeCell(node.label)} | ${node.maturity ?? ''} | ${node.orphan_score?.toFixed(2) ?? ''} |`
  ));
  const edgeRows = report.edges.map((edge) => (
    `| ${escapeCell(edge.from)} | ${escapeCell(edge.to)} | ${edge.type} | ${edge.strength.toFixed(2)} | ${escapeCell(edge.label)} |`
  ));
  const orphanRows = report.orphan_capabilities.map((orphan) => (
    `| ${escapeCell(orphan.capability_id)} | ${orphan.severity} | ${orphan.score.toFixed(2)} | ${escapeCell(orphan.reasons.join('; '))} | ${escapeCell(orphan.recommended_action)} |`
  ));
...
function nodeForConnection(connection: string): ProductCapabilityGraphNode {
...
function edgeTypeForConnection(connection: string): ProductCapabilityGraphEdgeType {
  if (connection.startsWith('learning:')) return 'learning';
  if (connection.startsWith('portfolio:')) return 'portfolio';
  return 'context';
}

function strengthForConnection(connection: string): number {
...
function orphanForCapability(
  capability: ProductTotalityCapability,
  peerDegree: number,
  capabilityCount: number,
): ProductCapabilityGraphOrphan | undefined {
  ...
  const hasSystemContext = capability.connections.some((connection) => (
    connection === 'roadmap' || connection === 'ecosystem' || connection === 'meaning'
  ));
  ...
  const score = clamp(
    (structuralConnections < 2 ? 0.35 : 0)
    + (!hasLearning ? 0.2 : 0)
    + (!hasPortfolio ? 0.15 : 0)
...
function sharedKeywords(left: ProductTotalityCapability, right: ProductTotalityCapability): string[] {
...
function graphKeywords(value: string): string[] {
...
function dedupeEdges(edges: ProductCapabilityGraphEdge[]): ProductCapabilityGraphEdge[] {
  const seen = new Set<string>();
  const out: ProductCapabilityGraphEdge[] = [];
  for (const edge of edges) {
