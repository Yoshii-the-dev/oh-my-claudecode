...
export function validateProductResearchArtifact(
  options: ValidateProductResearchArtifactOptions = {},
): ProductResearchArtifactValidationResult {
  const root = resolve(options.root ?? process.cwd());
  const relativePath = options.artifactPath ?? DEFAULT_RESEARCH_ARTIFACT_RELATIVE_PATH;
  const path = resolve(root, relativePath);
  const issues: ProductResearchArtifactIssue[] = [];
...
function extractResearchVerdict(content: string): ProductResearchVerdict {
  const yamlMatch = content.match(/(?:^|\n)research[_\s-]*verdict\s*:\s*(pass|blocked)\b/i);
  if (yamlMatch?.[1]) return yamlMatch[1].toLowerCase() as ProductResearchVerdict;

  const headingMatch = content.match(/(?:^|\n)#{1,6}\s*research verdict\s*\n\s*(pass|blocked)\b/i);
  if (headingMatch?.[1]) return headingMatch[1].toLowerCase() as ProductResearchVerdict;

  return 'unknown';
}

function countMatchingSections(
  content: string,
  sections: Array<{ pattern: RegExp }>,
): number {
  return sections.filter((section) => section.pattern.test(content)).length;
}

function countConcreteSources(content: string): number {
...
function extractFieldValue(content: string, field: string): string | undefined {
