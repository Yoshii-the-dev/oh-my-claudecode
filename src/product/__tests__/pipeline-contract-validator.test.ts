...
`,
  ...
  it('passes priority handoff when totality depth debt is carried into portfolio', () => {
    const root = createRoot();
    const portfolio = JSON.parse(portfolioLedgerArtifact()) as { items: Array<Record<string, unknown>> };
...
function writeFoundationLiteArtifacts(
  root: string,
  overrides: Partial<Record<'capability' | 'ecosystem' | 'portfolio' | 'opportunities' | 'roadmap' | 'meaning', string>> = {},
): void {
...
function capabilityMapArtifact(nextAgent = 'priority-engine'): string {
...
function ecosystemArtifact(): string {
...
function portfolioLedgerArtifact(count = 20): string {
...
function opportunitiesArtifact(options: { status?: string; count?: number } = {}): string {
...
function roadmapArtifact(): string {
  return `# Roadmap
...
function experienceGateArtifact(): string {
...
function cycleArtifact(stage: string, includeLearningReference = true): string {
...
function productRegressionAuditArtifact(): string {
  return JSON.stringify({
    schema_version: 1,
...
function scenarioCoverageAuditArtifact(): string {
  return JSON.stringify({
    schema_version: 1,
    generated_at: '2026-04-25T00:00:00.000Z',
    root: '/tmp/test',
...
function productTotalityAuditArtifact(): string {
  return JSON.stringify({
    schema_version: 1,
    generated_at: '2026-04-25T00:00:00.000Z',
    root: '/tmp/test',
...
function capabilityLifecycleAuditArtifact(): string {
  return JSON.stringify({
    schema_version: 1,
    generated_at: '2026-04-25T00:00:00.000Z',
    root: '/tmp/test',
...
function codes(report: ReturnType<typeof validateProductPipelineContracts>): string[] {
  return report.issues.map((issue) => issue.code);
}

function metric(
  report: ReturnType<typeof validateProductPipelineContracts>,
  artifactName: string,
  metricName: string,
