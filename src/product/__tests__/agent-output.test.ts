...
import {
  ...
  validateAgentOutput,
  renderAgentOutputMarkdown,
  resolveAgentOutputPath,
  type AgentOutput,
} from '../agent-output.js';

function validOutput(overrides: Partial<AgentOutput> = {}): AgentOutput {
  return {
...
describe('validateAgentOutput', () => {
  ...
  it('rejects non-object input', () => {
    ...
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'not-object' }),
    );
  });
  ...
  it('rejects missing agent_role', () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'missing-agent-role' }),
    );
  });
  ...
  it('rejects confidence out of range', () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'invalid-confidence' }),
    );
  });
  ...
  it('rejects empty artifacts_produced', () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'missing-artifacts-produced' }),
    );
  });
  ...
  it('requires halt block when status is halted', () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'missing-halt' }),
    );
  });
  ...
  it('rejects invalid signal type', () => {
    const result = validateAgentOutput({
      ...validOutput(),
      signals: { nested: { bad: 'value' } as unknown as number },
    });
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'invalid-signal-type' }),
    );
  });
  ...
  it('rejects invalid routing entry', () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'invalid-routing-agent' }),
    );
  });
  ...
  it('validates strategy.decision verdict', () => {
    const result = validateAgentOutput({
      ...validOutput(),
      strategy: {
        decision: { verdict: 'yolo' as 'approve', rationale: 'because' },
      },
    });
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'invalid-strategy-verdict' }),
    );
  });
  ...
  it('validates requires_user_input shape', () => {
    const result = validateAgentOutput({
      ...
      requires_user_input: [
        ...
        { question: 123 as unknown as string, blocking: true },
      ],
    });
    expect(result.ok).toBe(false);
    expect(result.issues).toContainEqual(
      expect.objectContaining({ code: 'invalid-user-input-question' }),
    );
  });
});
