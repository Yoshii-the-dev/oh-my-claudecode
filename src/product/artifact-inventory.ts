...
export function validateProductArtifactInventory(root = process.cwd()): ProductArtifactInventoryReport {
  const resolvedRoot = resolve(root);
  const artifactRoot = resolve(resolvedRoot, '.omc');
  const issues: ProductArtifactInventoryIssue[] = [];
  ...
  for (const file of productFiles) {
    if (isUnregisteredCurrentArtifact(file.relativePath)) {
      issues.push(issue(
        ...
        file.relativePath,
        'Current artifact is not in PRODUCT_ARTIFACT_REGISTRY; add it to the registry or archive it',
      ));
    }
  ...
  for (const [dir, count] of countByDirectory(productFiles).entries()) {
    if (count > STALE_PRESSURE_THRESHOLD) {
      issues.push(issue(
        ...
        dir,
        `${count} artifacts in one product directory; run artifact lifecycle/archive before downstream agents broad-scan this context`,
      ));
    }
  }
...
function collectArtifactFiles(dir: string, root: string): Array<{ path: string; relativePath: string; content: string }> {
  const out: Array<{ path: string; relativePath: string; content: string }> = [];
  walk(dir, root, out);
  return out;
}

function walk(dir: string, root: string, out: Array<{ path: string; relativePath: string; content: string }>): void {
  for (const entry of readdirSync(dir)) {
    const path = join(dir, entry);
    const stat = statSync(path);
    if (stat.isDirectory()) {
      if (entry === 'logs' || entry === 'state' || entry === 'archive') continue;
      walk(path, root, out);
      continue;
    }
    if (!stat.isFile() || !/\.(md|json|jsonc)$/i.test(path)) continue;
    const relativePath = relative(root, path);
    out.push({ path, relativePath, content: safeRead(path) });
  }
}

function isProductArtifactPath(relativePath: string): boolean {
  return PRODUCT_ARTIFACT_ALLOWED_DIRECTORIES.some((prefix) => relativePath.startsWith(prefix));
}

function isUnregisteredCurrentArtifact(relativePath: string): boolean {
  return /\/current\.(md|json|jsonc)$/i.test(relativePath)
    && !PRODUCT_ARTIFACT_CURRENT_PATHS.has(relativePath);
}

function shouldRequireFooter(relativePath: string): boolean {
  if (relativePath.endsWith('/current.md')) return true;
  if (relativePath === '.omc/constitution.md') return true;
  if (relativePath.includes('/handoffs/')) return true;
  if (relativePath.includes('/audits/')) return true;
...
function countByDirectory(files: Array<{ relativePath: string }>): Map<string, number> {
...
function makeReport(
  ...
  filesScanned: number,
  issues: ProductArtifactInventoryIssue[],
): ProductArtifactInventoryReport {
...
function issue(
  severity: ProductArtifactInventoryIssue['severity'],
  code: string,
  path: string,
  message: string,
): ProductArtifactInventoryIssue {
